...
while (<FILE>) {
	...
	if ($line =~ /^PERMISSION:(.*)/) {
		...
		if (!($line =~ /^\s*$/)) {
			push (@{$seeds{$current_permission."=".$line}{$current_type}}, $line) if ($current_permission ne "android.permission.BROADCAST_STICKY");
		}
	}
...
while (<FILE>) {
	...
	if ($line =~ /^PERMISSION:(.*)/) {
		...
		if (!($line =~ /^\s*$/)) {
			push (@{$seeds{$current_permission."=".$line}{$current_type}}, $line);
		}
	}
